...
meta:
  - name: description
    content: Open Data Product Recipe Specification (ODPR) version 1.0 defines delivery flows, product handoff flows, agent discovery flows, and trigger-based flows, with recipes, runtime profiles, and recipe catalogs as supporting building blocks.
  - name: spec-version
    content: "1.0"
  - name: llms
    content: /llms.txt
...
**ODPR has four primary composite flows.** It defines delivery flows for
...
product. It defines agent discovery flows, where AI agents investigate bounded
context to find grounded answers, findings, or review recommendations. It
defines trigger-based flows, where graph changes can make declared work
applicable.
...
* define agent discovery flows for bounded, grounded AI-agent investigation
...
ODPR solves this by giving teams and tools four composite flow contracts:
...
* which graph node and edge types should ground the work
...
ODPR has four root-level flow types.
...
**3. Agent discovery flows** declare how an AI agent investigates bounded
context to find grounded answers, findings, impact explanations, or review
recommendations.

**4. Trigger-based flows** declare when graph changes can make work applicable. They
...
four flow types. Their detailed YAML structures are described after the flow
sections.
...
* ODPR defines delivery flows, product handoff flows, agent discovery flows, and
  trigger-based flows around those artifacts.
...
Read the four flow sections first: Delivery Flows, Product Handoff Flows, Agent
Discovery Flows, and Trigger-Based Flows. Then use the support sections for the
reusable YAML building blocks: Recipe, RuntimeProfile, and RecipeCatalog.
